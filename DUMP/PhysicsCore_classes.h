...
struct UBodySetupCore : UObject {
	...
	enum class EPhysicsType PhysicsType; // 0x2c(0x01)
	enum class ECollisionTraceFlag CollisionTraceFlag; // 0x2d(0x01)
	enum class EBodyCollisionResponse CollisionReponse; // 0x2e(0x01)
...
struct UPhysicsSettingsCore : UDeveloperSettings {
	float DefaultGravityZ; // 0x30(0x04)
	float DefaultTerminalVelocity; // 0x34(0x04)
	float DefaultFluidFriction; // 0x38(0x04)
	int32_t SimulateScratchMemorySize; // 0x3c(0x04)
	int32_t RagdollAggregateThreshold; // 0x40(0x04)
	float TriangleMeshTriangleMinAreaThreshold; // 0x44(0x04)
	bool bEnableShapeSharing; // 0x48(0x01)
	bool bEnablePCM; // 0x49(0x01)
	bool bEnableStabilization; // 0x4a(0x01)
	bool bWarnMissingLocks; // 0x4b(0x01)
	bool bEnable2DPhysics; // 0x4c(0x01)
	bool bDefaultHasComplexCollision; // 0x4d(0x01)
	char pad_4E[0x2]; // 0x4e(0x02)
	float BounceThresholdVelocity; // 0x50(0x04)
	enum class EFrictionCombineMode FrictionCombineMode; // 0x54(0x01)
	enum class EFrictionCombineMode RestitutionCombineMode; // 0x55(0x01)
	char pad_56[0x2]; // 0x56(0x02)
	float MaxAngularVelocity; // 0x58(0x04)
	float MaxDepenetrationVelocity; // 0x5c(0x04)
	float ContactOffsetMultiplier; // 0x60(0x04)
	float MinContactOffset; // 0x64(0x04)
	float MaxContactOffset; // 0x68(0x04)
	bool bSimulateSkeletalMeshOnDedicatedServer; // 0x6c(0x01)
	enum class ECollisionTraceFlag DefaultShapeComplexity; // 0x6d(0x01)
