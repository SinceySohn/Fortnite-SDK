...
struct UDownloadOnDemandComponent : UControllerComponent {
	...
	void HandleMnemonicsFromValkyriePaletteProviderReceived(struct TArray<struct FString>& Mnemonics); // Function VKDownloadOnDemandGameplay.DownloadOnDemandComponent.HandleMnemonicsFromValkyriePaletteProviderReceived // (Final|Native|Private|HasOutParms) // @ game+0x6b93440
};
...
struct UDownloadOnDemandProject : UObject {
	...
	struct UDownloadOnDemandProjectDescriptor* Descriptor; // 0xb8(0x08)
	struct TMap<struct FString, struct UDownloadOnDemandProjectContent*> Contents; // 0xc0(0x50)
